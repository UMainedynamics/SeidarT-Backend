...
contains
    ...
    subroutine biotdg2(domain, source, SINGLE_OUTPUT)
        ...
        allocate(srcqx(source%time_steps), srcqz(source%time_steps), srcp(source%time_steps))
        ...
        else if (is_fluid_injection_source(source%source_type)) then
            call loadsource('seismicsourceqx.dat', source%time_steps, srcqx)
            call loadsource('seismicsourceqz.dat', source%time_steps, srcqz)
    ...
    subroutine biotdg25(domain, source, SINGLE_OUTPUT)
        ...
        if (source%source_type == 'ac') then
            ...
            call loadsource('seismicsourceqx.dat', source%time_steps, srcqx)
            call loadsource('seismicsourceqy.dat', source%time_steps, srcqy)
            call loadsource('seismicsourceqz.dat', source%time_steps, srcqz)
        else if (is_pressure_injection_source(source%source_type)) then
            call loadsource('seismicsourcep.dat', source%time_steps, srcp)
        else
