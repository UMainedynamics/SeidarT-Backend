module cpmlfdtd
    ...
    subroutine permittivity_write(im, mlist, npoints_pml, nx, nz) 
        ...
        integer,parameter :: dp = kind(0.d0)
        integer, intent(in) :: nx,nz, npoints_pml
        integer,dimension(nx,nz), intent(in) :: im
        integer :: i, j
        real(kind=dp), dimension(:,:), intent(in) :: mlist

        ! Declare allocatable arrays
        real(kind=kind(0.d0)), allocatable :: eps11(:,:), eps22(:,:), eps33(:,:)
        real(kind=kind(0.d0)), allocatable :: eps12(:,:), eps13(:,:), eps23(:,:)
        real(kind=kind(0.d0)), allocatable :: sig11(:,:), sig22(:,:), sig33(:,:)
        real(kind=kind(0.d0)), allocatable :: sig12(:,:), sig13(:,:), sig23(:,:)

        ! Allocate arrays based on input dimensions
        allocate(eps11(2*npoints_pml+nx, 2*npoints_pml+nz))
        allocate(eps12(2*npoints_pml+nx, 2*npoints_pml+nz))
        allocate(eps13(2*npoints_pml+nx, 2*npoints_pml+nz))
        allocate(eps22(2*npoints_pml+nx, 2*npoints_pml+nz))
        allocate(eps23(2*npoints_pml+nx, 2*npoints_pml+nz))
        allocate(eps33(2*npoints_pml+nx, 2*npoints_pml+nz))
        allocate(sig11(2*npoints_pml+nx, 2*npoints_pml+nz))
        allocate(sig12(2*npoints_pml+nx, 2*npoints_pml+nz))
        allocate(sig13(2*npoints_pml+nx, 2*npoints_pml+nz))
        allocate(sig22(2*npoints_pml+nx, 2*npoints_pml+nz))
        allocate(sig23(2*npoints_pml+nx, 2*npoints_pml+nz))
        allocate(sig33(2*npoints_pml+nx, 2*npoints_pml+nz))
        ...
        do i = 1,npoints_pml+1
            ...
            sig12( nx+npoints_pml-1+i, : ) = sig12(nx+npoints_pml-1,:)
    ...
    subroutine material_rw(filename, image_data, readfile)
        ...
        open(newunit=new_unit, form="unformatted", file = trim(filename))
