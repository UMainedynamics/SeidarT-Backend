module cpmlfdtd 
    ...
    subroutine seismic2(domain, source, density_method, SINGLE_OUTPUT)
        ...
        do it = 1,source%time_steps
            ...
            !$omp& shared(vx, vz, sigmaxx, sigmazz, memory_dvx_dx1, memory_dvx_dz1, memory_dvz_dz1, memory_dvz_dx1)
            !$omp do collapse(2)
#define LOOP_DIRECTIVE do
#else
            !$omp target teams loop collapse(2) private(value_dvx_dx, value_dvx_dz, value_dvz_dx, value_dvz_dz)
#define LOOP_DIRECTIVE do
#endif
            LOOP_DIRECTIVE j = 3,nz-1
                do i = 2,nx-2
    ...
    subroutine seismic3(domain, source, density_method, SINGLE_OUTPUT)
        ...
        allocate(c11(nx,ny,nz), c12(nx,ny,nz), c13(nx,ny,nz), c14(nx,ny,nz), c15(nx,ny,nz), c16(nx, ny, nz), &
                              c22(nx,ny,nz), c23(nx,ny,nz), c24(nx,ny,nz), c25(nx,ny,nz), c26(nx, ny, nz), &
                                           c33(nx,ny,nz), c34(nx,ny,nz), c35(nx,ny,nz), c36(nx, ny, nz), &
            ...
            do k = 3,nz-1
                do j = 3,ny-1
                    do i = 2,nx-2
                        ...
                        memory_dvx_dx1(i,j,k) = bcoef_half(i,j,k) * memory_dvx_dx1(i,j,k) + acoef_half(i,j,k) * dvx_dx
                        memory_dvy_dx1(i,j,k) = bcoef_half(i,j,k) * memory_dvy_dx1(i,j,k) + acoef_half(i,j,k) * dvy_dx
                        memory_dvz_dx1(i,j,k) = bcoef_half(i,j,k) * memory_dvz_dx1(i,j,k) + acoef_half(i,j,k) * dvz_dx
                        memory_dvy_dy1(i,j,k) = bcoef(i,j,k) * memory_dvy_dy1(i,j,k) + acoef(i,j,k) * dvy_dy
                        memory_dvx_dy1(i,j,k) = bcoef(i,j,k) * memory_dvx_dy1(i,j,k) + acoef(i,j,k) * dvx_dy
                        memory_dvz_dy1(i,j,k) = bcoef(i,j,k) * memory_dvz_dy1(i,j,k) + acoef(i,j,k) * dvz_dy
                        memory_dvz_dz1(i,j,k) = bcoef(i,j,k) * memory_dvz_dz1(i,j,k) + acoef(i,j,k) * dvz_dz
                        memory_dvx_dz1(i,j,k) = bcoef(i,j,k) * memory_dvx_dz1(i,j,k) + acoef(i,j,k) * dvx_dz
                        memory_dvy_dz1(i,j,k) = bcoef(i,j,k) * memory_dvy_dz1(i,j,k) + acoef(i,j,k) * dvy_dz

                        dvx_dx = dvx_dx / kappa_half(i,j,k) + memory_dvx_dx1(i,j,k)
                        dvy_dx = dvy_dx / kappa_half(i,j,k) + memory_dvy_dx1(i,j,k)
                        dvz_dx = dvz_dx / kappa_half(i,j,k) + memory_dvz_dx1(i,j,k)
                        dvy_dy = dvy_dy / kappa(i,j,k) + memory_dvy_dy1(i,j,k)
                        dvx_dy = dvx_dy / kappa(i,j,k) + memory_dvx_dy1(i,j,k)
                        dvz_dy = dvz_dy / kappa(i,j,k) + memory_dvz_dy1(i,j,k)
                        dvz_dz = dvz_dz / kappa(i,j,k) + memory_dvz_dz1(i,j,k)
                        dvx_dz = dvx_dz / kappa(i,j,k) + memory_dvx_dz1(i,j,k)
                        dvy_dz = dvy_dz / kappa(i,j,k) + memory_dvy_dz1(i,j,k)

                        sigmaxx(i,j,k) = ( sigmaxx(i,j,k) + &
                        (   c11(i,j,k) * dvx_dx + c12(i,j,k) * dvy_dy + c13(i,j,k) * dvz_dz + &
                            c14(i,j,k) * (dvy_dz + dvz_dy) + c15(i,j,k) * (dvx_dz + dvz_dx) + &
                            c16(i,j,k) * (dvy_dx + dvx_dy) ) * dt ) / &
                                (1 + gamma_x(i,j,k) * dt )

                        ! Full 3D will need a gradient in the y-direction
                        sigmayy(i,j,k) = ( sigmayy(i,j,k) + &
                        (   c12(i,j,k) * dvx_dx + c22(i,j,k) * dvy_dy + c23(i,j,k) * dvz_dz + &
                            c24(i,j,k) * (dvy_dz + dvz_dy) + c25(i,j,k) * (dvx_dz + dvz_dx) + &
                            c26(i,j,k) * (dvy_dx + dvx_dy) ) * dt ) / &
                                (1 + gamma_y(i,j,k) * dt )

                        sigmazz(i,j,k) = ( sigmazz(i,j,k) + &
                        (   c13(i,j,k) * dvx_dx + c23(i,j,k) * dvy_dy + c33(i,j,k) * dvz_dz + &
                            c34(i,j,k) * (dvy_dz + dvz_dy) + c35(i,j,k) * (dvx_dz + dvz_dx) + &
                            c36(i,j,k) * (dvy_dx + dvx_dy) ) * dt ) / &
                                (1 + gamma_z(i,j,k) * dt )
            ...
            do k = 3,nz-1
                do j = 2,ny-2
                    do i = 3,nx-1
                        ...
                        memory_dvx_dx2(i,j,k) = bcoef(i,j,k) * memory_dvx_dx2(i,j,k) + acoef(i,j,k) * dvx_dx
                        memory_dvy_dx2(i,j,k) = bcoef(i,j,k) * memory_dvy_dx2(i,j,k) + acoef(i,j,k) * dvy_dx
                        memory_dvz_dx2(i,j,k) = bcoef(i,j,k) * memory_dvz_dx2(i,j,k) + acoef(i,j,k) * dvz_dx
                        memory_dvy_dy2(i,j,k) = bcoef_half(i,j,k) * memory_dvy_dy2(i,j,k) + acoef_half(i,j,k) * dvy_dy
                        memory_dvx_dy2(i,j,k) = bcoef_half(i,j,k) * memory_dvx_dy2(i,j,k) + acoef_half(i,j,k) * dvx_dy
                        memory_dvz_dy2(i,j,k) = bcoef_half(i,j,k) * memory_dvz_dy2(i,j,k) + acoef_half(i,j,k) * dvz_dy
                        memory_dvz_dz2(i,j,k) = bcoef_half(i,j,k) * memory_dvz_dz2(i,j,k) + acoef_half(i,j,k) * dvz_dz
                        memory_dvx_dz2(i,j,k) = bcoef_half(i,j,k) * memory_dvx_dz2(i,j,k) + acoef_half(i,j,k) * dvx_dz
                        memory_dvy_dz2(i,j,k) = bcoef_half(i,j,k) * memory_dvy_dz2(i,j,k) + acoef_half(i,j,k) * dvy_dz

                        dvx_dx = dvx_dx / kappa(i,j,k) + memory_dvx_dx2(i,j,k)
                        dvy_dx = dvy_dx / kappa(i,j,k) + memory_dvy_dx2(i,j,k)
                        dvz_dx = dvz_dx / kappa(i,j,k) + memory_dvz_dx2(i,j,k)
                        dvy_dy = dvy_dy / kappa_half(i,j,k) + memory_dvy_dy2(i,j,k)
                        dvx_dy = dvx_dy / kappa_half(i,j,k) + memory_dvx_dy2(i,j,k)
                        dvz_dy = dvz_dy / kappa_half(i,j,k) + memory_dvz_dy2(i,j,k)
                        dvz_dz = dvz_dz / kappa_half(i,j,k) + memory_dvz_dz2(i,j,k)
                        dvy_dz = dvy_dz / kappa_half(i,j,k) + memory_dvy_dz2(i,j,k)

                        sigmaxy(i,j,k) = ( sigmaxy(i,j,k) + &
                        (   c16(i,j,k) * dvx_dx + c26(i,j,k) * dvy_dy + c36(i,j,k) * dvz_dz + &
                            c46(i,j,k) * (dvy_dz + dvz_dy) + c56(i,j,k) * (dvx_dz + dvz_dx) + &
            ...
            do k = 2,nz-2
                do j = 3,ny-1
                    do i = 3,nx-1
                        ...
                        memory_dvx_dx3(i,j,k) = bcoef(i,j,k) * memory_dvx_dx3(i,j,k) + acoef(i,j,k) * dvx_dx
                        memory_dvy_dx3(i,j,k) = bcoef(i,j,k) * memory_dvy_dx3(i,j,k) + acoef(i,j,k) * dvy_dx
                        memory_dvz_dx3(i,j,k) = bcoef(i,j,k) * memory_dvz_dx3(i,j,k) + acoef(i,j,k) * dvz_dx
                        memory_dvy_dy3(i,j,k) = bcoef(i,j,k) * memory_dvy_dy3(i,j,k) + acoef(i,j,k) * dvy_dy
                        memory_dvx_dy3(i,j,k) = bcoef(i,j,k) * memory_dvx_dy3(i,j,k) + acoef(i,j,k) * dvx_dy
                        memory_dvz_dy3(i,j,k) = bcoef(i,j,k) * memory_dvz_dy3(i,j,k) + acoef(i,j,k) * dvz_dy
                        memory_dvz_dz3(i,j,k) = bcoef_half(i,j,k) * memory_dvz_dz3(i,j,k) + acoef_half(i,j,k) * dvz_dz
                        memory_dvx_dz3(i,j,k) = bcoef_half(i,j,k) * memory_dvx_dz3(i,j,k) + acoef_half(i,j,k) * dvx_dz
                        memory_dvy_dz3(i,j,k) = bcoef_half(i,j,k) * memory_dvy_dz3(i,j,k) + acoef_half(i,j,k) * dvy_dz

                        dvx_dx = dvx_dx / kappa(i,j,k) + memory_dvx_dx3(i,j,k)
                        dvy_dx = dvy_dx / kappa(i,j,k) + memory_dvy_dx3(i,j,k)
                        dvz_dx = dvz_dx / kappa(i,j,k) + memory_dvz_dx3(i,j,k) 
                        dvy_dy = dvy_dy / kappa(i,j,k) + memory_dvy_dy3(i,j,k)
                        dvx_dy = dvx_dy / kappa(i,j,k) + memory_dvx_dy3(i,j,k)
                        dvz_dy = dvz_dy / kappa(i,j,k) + memory_dvz_dy3(i,j,k)
                        dvz_dz = dvz_dz / kappa_half(i,j,k) + memory_dvz_dz3(i,j,k)
                        dvx_dz = dvx_dz / kappa_half(i,j,k) + memory_dvx_dz3(i,j,k)
                        dvy_dz = dvy_dz / kappa_half(i,j,k) + memory_dvy_dz3(i,j,k)

                        sigmaxz(i,j,k) = ( sigmaxz(i,j,k) + &
                            (   c15(i,j,k) * dvx_dx + c25(i,j,k) * dvy_dy + c35(i,j,k) * dvz_dz + &
                                c45(i,j,k) * ( dvy_dz + dvz_dy) + c55(i,j,k) * ( dvx_dz + dvz_dx) + &
                                c56(i,j,k) * ( dvy_dx + dvx_dy) ) * dt  ) / &
                                    (1 + gamma_xz(i,j,k) * dt )
                ...
                do j = 2,ny-2
                    do i = 2,nx-2
                        ...
                        memory_dvx_dx4(i,j,k) = bcoef_half(i,j,k) * memory_dvx_dx4(i,j,k) + acoef_half(i,j,k) * dvx_dx
                        memory_dvy_dx4(i,j,k) = bcoef_half(i,j,k) * memory_dvy_dx4(i,j,k) + acoef_half(i,j,k) * dvy_dx
                        memory_dvz_dx4(i,j,k) = bcoef_half(i,j,k) * memory_dvz_dx4(i,j,k) + acoef_half(i,j,k) * dvz_dx
                        memory_dvy_dy4(i,j,k) = bcoef_half(i,j,k) * memory_dvy_dy4(i,j,k) + acoef_half(i,j,k) * dvy_dy
                        memory_dvx_dy4(i,j,k) = bcoef_half(i,j,k) * memory_dvx_dy4(i,j,k) + acoef_half(i,j,k) * dvx_dy
                        memory_dvz_dy4(i,j,k) = bcoef_half(i,j,k) * memory_dvz_dy4(i,j,k) + acoef_half(i,j,k) * dvz_dy
                        memory_dvz_dz4(i,j,k) = bcoef_half(i,j,k) * memory_dvz_dz4(i,j,k) + acoef_half(i,j,k) * dvz_dz
                        memory_dvx_dz4(i,j,k) = bcoef_half(i,j,k) * memory_dvx_dz4(i,j,k) + acoef_half(i,j,k) * dvx_dz
                        memory_dvy_dz4(i,j,k) = bcoef_half(i,j,k) * memory_dvy_dz4(i,j,k) + acoef_half(i,j,k) * dvy_dz

                        dvx_dx = dvx_dx / kappa_half(i,j,k) + memory_dvx_dx4(i,j,k)
                        dvy_dx = dvy_dx / kappa_half(i,j,k) + memory_dvy_dx4(i,j,k)
                        dvz_dx = dvz_dx / kappa_half(i,j,k) + memory_dvz_dx4(i,j,k)
                        dvy_dy = dvy_dy / kappa_half(i,j,k) + memory_dvy_dy4(i,j,k)
                        dvx_dy = dvx_dy / kappa_half(i,j,k) + memory_dvx_dy4(i,j,k)
                        dvz_dy = dvz_dy / kappa_half(i,j,k) + memory_dvz_dy4(i,j,k)
                        dvz_dz = dvz_dz / kappa_half(i,j,k) + memory_dvz_dz4(i,j,k)
                        dvx_dz = dvx_dz / kappa_half(i,j,k) + memory_dvx_dz4(i,j,k)
...
#define LOOP_DIRECTIVE do
#else
            !$omp target teams loop collapse(2) private(dEx_dz, dEz_dx)
#define LOOP_DIRECTIVE do
#endif
            LOOP_DIRECTIVE j = 1,nz-1 
                do i = 1,nx-1
            ...
            !$omp do
#else
            !$omp target teams loop collapse(2) private(dHy_dz, dHy_dx)
#endif
            LOOP_DIRECTIVE j = 2,nz
                do i = 2,nx
...
!!!!!!!!!!!!!!!! End
    ...
    subroutine electromag3(domain, source, SINGLE_OUTPUT)
        ...
        ! 1D arrays for the damping profiles in each direction
        real(real64), allocatable :: kappa(:,:,:), alpha(:,:,:),  &
                                     acoef(:,:,:), bcoef(:,:,:), &
                                     kappa_half(:,:,:), alpha_half(:,:,:), &
                                     acoef_half(:,:,:), bcoef_half(:,:,:)
        ...
        dz = domain%dz
        dt = source%dt
        allocate(eps11(nx,ny,nz), eps12(nx,ny,nz), eps13(nx,ny,nz), &
                    eps22(nx,ny,nz), eps23(nx,ny,nz), eps33(nx,ny,nz))
        allocate(sig11(nx,ny,nz), sig12(nx,ny,nz), sig13(nx,ny,nz), &
                    sig22(nx,ny,nz), sig23(nx,ny,nz), sig33(nx,ny,nz))
        
        allocate(alpha(nx,ny,nz), kappa(nx,ny,nz), &
                 acoef(nx,ny,nz), bcoef(nx,ny,nz) )         
        allocate(alpha_half(nx,ny,nz), kappa_half(nx,ny,nz), &
                 acoef_half(nx,ny,nz), bcoef_half(nx,ny,nz) )

        allocate(srcx(source%time_steps), srcy(source%time_steps), srcz(source%time_steps))
        
        ! Allocate more
        allocate(aEx(nx,ny,nz), bEx(nx,ny,nz), cEx(nx,ny,nz), &
                 aEy(nx,ny,nz), bEy(nx,ny,nz), cEy(nx,ny,nz), &
                 aEz(nx,ny,nz), bEz(nx,ny,nz), cEz(nx,ny,nz))
        allocate(det(nx,ny,nz))
        allocate( memory_dEx_dy(nx,ny,nz), memory_dEy_dx(nx,ny,nz), &
                  memory_dEx_dz(nx,ny,nz), memory_dEz_dx(nx,ny,nz), &
                  memory_dEz_dy(nx,ny,nz), memory_dEy_dz(nx,ny,nz) )
        allocate( memory_dHz_dx(nx,ny,nz), memory_dHx_dz(nx,ny,nz), & 
            ...
            do k = 1,nz-1
                do i = 1,nx-1  
                    do j = 1,ny-1
                        ...
                        dEz_dy = dEz_dy/ kappa_half(i,j,k) + memory_dEz_dy(i,j,k)
                        dEy_dz = dEy_dz/ kappa_half(i,j,k) + memory_dEy_dz(i,j,k)
            ...
            do k = 1,nz-1
                do i = 1,nx-1      
                    do j = 1,ny-1
                        ...
                        memory_dEx_dz(i,j,k) = bcoef(i,j,k) * memory_dEx_dz(i,j,k) + acoef(i,j,k) * dEx_dz
                        memory_dEz_dx(i,j,k) = bcoef(i,j,k) * memory_dEz_dx(i,j,k) + acoef(i,j,k) * dEz_dx
                        
                        dEx_dz = dEx_dz/ kappa(i,j,k) + memory_dEx_dz(i,j,k)
                        dEz_dx = dEz_dx/ kappa(i,j,k) + memory_dEz_dx(i,j,k)
            ...
            do k = 2,nz-1
                do i = 1,nx-1      
                    do j = 1,ny-1
                        ...
                        memory_dEx_dy(i,j,k) = bcoef(i,j,k) * memory_dEx_dy(i,j,k) + acoef(i,j,k) * dEx_dy
                        memory_dEy_dx(i,j,k) = bcoef(i,j,k) * memory_dEy_dx(i,j,k) + acoef(i,j,k) * dEy_dx
                        
                        dEx_dy = dEx_dy/ kappa(i,j,k) + memory_dEx_dy(i,j,k)
                        dEy_dx = dEy_dx/ kappa(i,j,k) + memory_dEy_dx(i,j,k)
            ...
            do k = 2,nz-1 
                do i = 2,nx-1
                    do j = 2,ny-1 
                        ...
                        dHz_dx = dHz_dx/kappa_half(i,j,k) + memory_dHz_dx(i,j,k)
                        dHx_dz = dHx_dz/kappa_half(i,j,k) + memory_dHx_dz(i,j,k)
                        ...
                        Ez(i,j,k) = Ez_old(i,j,k) + ( aEz(i,j,k) * rhs_x + bEz(i,j,k) * rhs_y + cEz(i,j,k) * rhs_z ) * dt
                    end do 
                end do 
            end do 
            !$omp end parallel do
