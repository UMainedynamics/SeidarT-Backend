program main 
    ...
    type(Stiffness_Type) :: stiffness 
    type(Attenuation_Type) :: attenuation 
    type(Permittivity_Type) :: permittivity 
    type(Conductivity_Type) :: conductivity
    ...
    if (seismic) then 
        ...
        seismic_parameter_write(domain, geometry, stiffness, attenuation)
    else 
        print *, "Writing electromagnetic model parameters to Fortran unformatted binary files."
        electromagnetic_parameter_write(domain, geometry, permittivity, conductivity)
    end if
